...
def Collocations_Method_5(_bing_api_key, _n_grams_from_input_text_file, _input_file_path, _input_queries_hit_results, _individual_word_hit_results, _corpus, original_input_queries_file, individual_words, _verbose):
	...
	values_of_c = [1.0, 0.01, 0.1, 10, 100]
	string_values_of_c = ['1.0', '0.01', '0.1', '10', '100']
	_c_value = 1
	...
	for i in range(5):
		...
		_c_value = values_of_c[i]
		string_c = string_values_of_c[i]
	...
	Probability \t\t Product of individual word probabilities", file = _output_file_verbose)
		for n_gram in phrase_probabilities:
			...
			if phrase_probabilities[n_gram] > _c_value * product_of_individual_word_probabilities:
		...
		for n_gram in phrase_probabilities:
			...
			if phrase_probabilities[n_gram] > _c_value * value_to_multiply_product_of_individual_probabilities * product_of_individual_word_probabilities:
		...
		collocations_without_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], string_c + '/' + _corpus + '_collocations_probability_T1.txt')
		...
		n_grams_without_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], string_c + '/' + _corpus + '_not_collocations_probability_T1.txt')
		...
		collocations_with_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], string_c + '/' + _corpus + '_collocations_probability_T2.txt')
		...
		n_grams_with_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], string_c + '/' + _corpus + '_not_collocations_probability_T2.txt')
