...
from bing_search_result_totals import bing_search_total
...
def Collocations_Method_5(_bing_api_key, _n_grams_from_input_text_file, _input_file_path, _input_queries_hit_results, _individual_word_hit_results, _corpus, original_input_queries_file, individual_words, _verbose):
	...
	if _corpus == 'Bing':
		Universe_of_the_webpages = bing_search_total(False, 'a', _bing_api_key) # A total search result count from Bing.com of the word 'a'
...
Probability \t\t Product of individual word probabilities", file = _output_file_verbose)
	for n_gram in phrase_probabilities:
		...
		if phrase_probabilities[n_gram] > product_of_individual_word_probabilities:
	...
	for n_gram in phrase_probabilities:
		...
		if phrase_probabilities[n_gram] > value_to_multiply_product_of_individual_probabilities * product_of_individual_word_probabilities:
	...
	collocations_without_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], _corpus + '_collocations_probability_T1.txt')
	...
	n_grams_without_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], _corpus + '_not_collocations_probability_T1.txt')
	...
	collocations_with_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], _corpus + '_collocations_probability_T2.txt')
	...
	n_grams_with_uniqueness_output_file_path = str(_input_file_path).replace(_input_file_path.split('/')[-1], _corpus + '_not_collocations_probability_T2.txt')
