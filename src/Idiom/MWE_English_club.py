...
- Extracts MWEs given collocations
	This script does not take the POS of the individual words into consideration while extracting definitions from WordNet
	Definitions of Idioms are extracted from the text file Definitions.txt rather that from WordNet
- Steps:
...
import nltk, snowballstemmer, itertools, copy, re, os, unicodedata, sys#, Collocations_NVAA #, pos_collocations
...
import gcide_xml # import extract_definitions # Webster dictionary definitions form gcide_xml.py

def mwe(collocations, debug, key, intersection_union, output_file, idiom_dict_1):
	debug = debug # debug option
	key = key # Bing search API key
	option = intersection_union # U | I | B
	...
	webster_dictionary = gcide_xml.extract_definitions() # Webster dictionary definitions
	...
	for x in xrange(0,l):
		...
		c = re.sub(r'_.*? ', ' ', collocations[x] + ' ') # Removes POS tags attached to each word in the collocation
		###_dict_c = c.strip(' ').strip('\n').replace(' ','').lower()
		# For VNC
		_dict_c = c.rstrip(' ').replace(' ', '_').lower()
		print "collocation: ", _dict_c
		try:
			print idiom_dict_1[_dict_c]
		except KeyError:
			pass

		c_mechanize = c.rstrip(' ')
		...
		if c in all_definitions_returned: # Checks if the meanings of the collocation are already obtained
			...
			if _dict_c in idiom_dict_1:
				def_c = [idiom_dict_1[_dict_c.lower()]]
				if def_c == '':
					def_c == definitions_no_pos(c_wordnet, debug)
			else:
				def_c = definitions_no_pos(c_wordnet.lower(), debug) # To extract definitions from WordNet dictionary without considering thr POS of the collocation
			
			if ((def_c == 0) or (len(def_c) == 0) or def_c == [''] or def_c == ""): # i.e. if WordNet does not have any definitions
		...
		for y in xrange(0,len(w)): # POS tags the individual words of collocation
		...
		for z in xrange(0,len(w)):
			...
			if(word_with_pos in all_definitions_returned):
				...
				#lists[z] = definitions(w[z], tag, debug) # Considering POS of the word
				lists[z] = definitions_no_pos(w[z], debug) # Not considering POS of the word
		...
		for i in def_c:
			definition = str(i)
			def_c_new.append(c + " means " + definition)
		
		for j in xrange(0,len(lists)):
			individual_word = w[j]
			word_list = copy.deepcopy(lists[j])
			lists_new[j] = []
			for k in xrange(0, len(word_list)):
				definition = str(word_list[k])
				lists_new[j].append(individual_word + " means " + definition)
		...
		for a in def_c_new:
			...
			for b in xrange(0, (len(pos_tags) - n_in_ngram - 1)):
		...
		for a_list in lists_new:
			...
			for a_definion in a_list:
				...
				for b in xrange(0, (len(pos_tags) - 2)):
		...
		d_list = copy.deepcopy(d.items())
		d_list_counter = [] # A list to store the counter values
		for d_list_item in xrange(0, len(d_list)):
...
#collocations.append('ethnic group')
...
intersection_union = str(sys.argv[1]).upper() #raw_input().upper()
if (len(intersection_union) == 1):
	...
	if (intersection_union == 'I' or intersection_union == 'U' or intersection_union == 'B'):
		...
		"""
		idioms_with_def = open("/Users/vasanthi/Desktop/Dictionaries/English_Club/Idioms.txt")
		idioms_with_definitions = idioms_with_def.readlines()
		idioms_with_def.close()
		print len(idioms_with_definitions)

		idiom_def = open("/Users/vasanthi/Desktop/Dictionaries/English_Club/Definitions.txt")
		idiom_definitions = idiom_def.readlines()
		idiom_def.close()
		print len(idiom_definitions)

		idiom_dict_1 = {}
		for x in xrange(0,len(idioms_with_definitions)):
			idiom_dict_1[idioms_with_definitions[x].strip(' ').strip('\n').replace(' ','').replace("'","").replace('-',' ').replace(".","").lower()] = idiom_definitions[x]

		print idiom_dict_1
		#"""
		# VNC tokens
		#tokens = open("/Users/vasanthi/Desktop/Dictionaries/English_VNC_Cook/MWE_Extraction/Textractor_May_15/Unique_VNC_tokens.txt").readlines()
		#definitions = open("/Users/vasanthi/Desktop/Dictionaries/English_VNC_Cook/MWE_Extraction/Textractor_May_15/Unique_VNC_definitions.txt_ascii").readlines()
		
		# Wiktionary paper
		#tokens = open("/Users/vasanthi/Desktop/Dictionaries/English_VNC_Cook/MWE_Extraction/Wiktionary_tests_May_15/idioms_missed.txt").readlines()
		#definitions = open("/Users/vasanthi/Desktop/Dictionaries/English_VNC_Cook/MWE_Extraction/Wiktionary_tests_May_15/definitions_idioms_missed.txt").readlines()

		# English club - Idiom example sentences
		#tokens = open("/Users/vasanthi/Desktop/Dictionaries/English_Club/May_15_2015_RANLP_tests/Idioms.txt").readlines()
		#definitions = open("/Users/vasanthi/Desktop/Dictionaries/English_Club/May_15_2015_RANLP_tests/Definitions.txt").readlines()

		# 176 oxford idioms - ranlp 2015
		tokens = open("/Users/vasanthi/Desktop/Dictionaries/Oxford_Idioms/Tests/Idioms_only/RANLP_May_15_tests/Textractor_176/176_Idioms.punct_stripped.txt").readlines()
		definitions = open("/Users/vasanthi/Desktop/Dictionaries/Oxford_Idioms/Tests/Idioms_only/RANLP_May_15_tests/Textractor_176/176_definitions.txt").readlines()
		idiom_dict_1 = {}
		for x in xrange(0, len(tokens)):
			if not definitions[x].strip('\n') == ' ':
				idiom_dict_1[tokens[x].strip('\n').rstrip(' ').replace(' ', '_').lower()] = definitions[x].strip('\n')
		print idiom_dict_1

		#(collocations, output_file) = Collocations_NVAA.main() # calls the main function of pos_collocations.py
		#"""
		#print "Enter input file name:"
		infile = str(sys.argv[2]) #raw_input()
		ifile = open(infile, 'r')
		...
		output_file = str(sys.argv[3]) #raw_input()
		#"""
		"""	
		...
		#"""
		mwe(collocations, debug, key, intersection_union, output_file, idiom_dict_1)
	else:
