...
def mwe(collocations, debug, key, intersection_union, output_file, idiom_dict_1, idiom_dict_2):
	...
	for x in xrange(0,l):
		...
		c = re.sub('\_[^ ]* ', ' ', collocations[x]) # Removes POS tags attached to each word in the collocation
		_dict_c = c.strip(' ').strip('\n').replace(' ','')
		print _dict_c
		try:
			print idiom_dict_2[_dict_c]
		...
		if c in all_definitions_returned: # Checks if the meanings of the collocation are already obtained
			...
			if _dict_c in idiom_dict_1:
				def_c = [idiom_dict_1[_dict_c]]
			elif _dict_c in idiom_dict_2:
				def_c = idiom_dict_2[_dict_c]
			else:
				def_c = 0
...
intersection_union = raw_input().upper()
if (len(intersection_union) == 1):
	...
	if (intersection_union == 'I' or intersection_union == 'U' or intersection_union == 'B'):
		...
		#"""
		idioms_with_def = open(raw_input())
		idioms_with_definitions = idioms_with_def.readlines()
		idioms_with_def.close()
		print len(idioms_with_definitions)

		idiom_def = open(raw_input())
		idiom_definitions = idiom_def.readlines()
		idiom_def.close()
		print len(idiom_definitions)

		_1450_ = open(raw_input())
		_1450_idioms =_1450_.readlines()
		_1450_.close()
		print len(_1450_idioms)

		_1450_ = open(raw_input())
		_1450_definitions = _1450_.readlines()
		_1450_.close()
		_1450_idiom_definitions = []
		for i in _1450_definitions:
			_1450_idiom_definitions += [i.lstrip('=>').strip('\n').strip('...').split('=>')]
		print len(_1450_idiom_definitions)

		idiom_dict_1 = {}
		for x in xrange(0,len(idiom_definitions)):
			idiom_dict_1[idioms_with_definitions[x].strip(' ').strip('\n').replace(' ','')] = idiom_definitions[x]

		idiom_dict_2 = {}
		for x in xrange(0,len(_1450_idioms)):
			idiom_dict_2[_1450_idioms[x].strip(' ').strip('\n').replace(' ','')] = _1450_idiom_definitions[x]
		print idiom_dict_2
		...
		output_file = raw_input()
		#"""
		mwe(collocations, debug, key, intersection_union, output_file, idiom_dict_1, idiom_dict_2)
	else:
