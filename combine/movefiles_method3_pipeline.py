...
while doCalculation:
	...
	count = 0
	number_of_instances = len(string_values_of_c)
	...
	for c_string in string_values_of_c:
		...
		_folder_dest = formatPath(path_dest + "/{article}/c{c}/".format(article = folder_name, c = c_string))
		...
		print(len(unmodifiedExtSet_1_2))
		print(len(unmodifiedExtSet))

		for i in range(start, end + 1):
			#method 5 file
			s_path = formatPath(path_source + "/n-{n}/{article}/c{c}/{filename}.txt".format(n = str(i), article = folder_name, 
				c = c_string, filename = file_name))
	...
	with open(highest_result, 'w') as f:
		...
		print("C: {c}".format(c = highest_c_unm))
		...
		print("C: {c}".format(c = highest_c_sub))
